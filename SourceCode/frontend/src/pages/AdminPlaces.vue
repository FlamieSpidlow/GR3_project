      ...
      <div class="admin-container tw-container-wide page-body">
        ...
        <div class="search-section">
          ...
          <div v-if="searchResults.length > 0" class="search-results">
            <h4>Kết quả tìm kiếm ({{ searchResults.length }} địa điểm chưa có trong danh sách):</h4>
        ...
        <div class="tag-admin-section">
          <div class="tag-admin-header">
            <div>
              <h3>Chủ đề địa điểm</h3>
              <p>Thêm chủ đề mới để sử dụng khi tạo hoặc chỉnh sửa địa điểm.</p>
            </div>
          </div>
          <div class="tag-admin-form">
            ...
            <button class="btn btn-primary" type="button" @click="addTag" :disabled="isAddingTag">
              {{ isAddingTag ? 'Đang thêm...' : 'Thêm chủ đề' }}
            </button>
          </div>
          <div class="tag-admin-list" v-if="availableTags.length > 0">
            <span v-for="tag in visibleAdminTags" :key="tag" class="tag-pill">{{ tag }}</span>
            <button
              v-if="availableTags.length > tagPreviewLimit"
              type="button"
              class="tag-more-btn"
              @click="showAllTags = !showAllTags"
            >
              {{ showAllTags ? 'Thu gọn' : `Xem thêm ${availableTags.length - tagPreviewLimit} chủ đề` }}
            </button>
          </div>
        </div>
          ...
          <div v-else class="submissions-grid">
            <div v-for="s in pendingReviewImageSubmissions" :key="s._id" class="submission-card">
              ...
              <div class="submission-info">
                ...
                <div class="submission-time">Gửi lúc: {{ formatDateTime(s.createdAt) }}</div>
              </div>
        ...
        <h3>Danh sách địa điểm ({{ places.length }})</h3>
        ...
        <div v-else class="places-grid">
          <div v-for="place in places" :key="place._id" class="place-card">
            <div class="place-image">
              ...
              <span v-if="place.images && place.images.length > 1" class="image-count">{{ place.images.length }} ảnh</span>
            </div>
            <div class="place-info">
              <h3>{{ place.name }}</h3>
              <p class="place-address">{{ cleanAddress(place.address, place.name) || 'Chưa có địa chỉ' }}</p>
              <div class="place-meta">
                <span class="meta-tag">Độ tuổi {{ place.ageRange || '0-12' }}</span>
                <span class="meta-tag meta-price">{{ formatPrice(place.price) }}</span>
              </div>
            </div>
            <div class="place-actions">
              <button @click="editPlace(place)" class="btn-small btn-edit">Sửa</button>
              <button @click="confirmDelete(place)" class="btn-small btn-delete">Xóa</button>
            </div>
          </div>
        </div>
              ...
              <div class="form-group">
                ...
                <div class="inline-tag-create">
                  <input
                    ...
                    type="text"
                    placeholder="Nhập chủ đề mới cho địa điểm"
                  ...
                  <button type="button" class="btn btn-secondary" @click="addTagToPlaceForm" :disabled="isAddingPlaceTag">
                    {{ isAddingPlaceTag ? 'Đang thêm...' : '+ Chủ đề mới' }}
                  </button>
                </div>
...
export default {
  ...
  computed: {
    visibleAdminTags() {
      return this.showAllTags ? this.availableTags : this.availableTags.slice(0, this.tagPreviewLimit)
    }
  },
  mounted() {
    this.checkAdmin()
  ...
  methods: {
    ...
    async addTag() {
      ...
      if (!name) {
        this.$notify({ type: 'warning', title: 'Thiếu tên chủ đề', message: 'Vui lòng nhập tên chủ đề cần thêm.' })
        return
      }
      ...
      if (res && res.success && res.data) {
        ...
        this.newTagName = ''
        this.$notify({ type: 'success', title: 'Đã thêm chủ đề', message: `Chủ đề "${tagName}" đã được thêm.` })
      } else {
        this.$notify({ type: 'error', title: 'Không thể thêm chủ đề', message: res?.error || 'Đã có lỗi xảy ra.' })
      }
    },
    async addTagToPlaceForm() {
      ...
      if (res && res.success && res.data) {
        ...
        this.formData.tags = this.mergeTags(this.formData.tags, [tagName])
        this.placeTagName = ''
        this.$notify({ type: 'success', title: 'Đã thêm chủ đề', message: `Chủ đề "${tagName}" đã được thêm vào địa điểm.` })
      } else {
        this.$notify({ type: 'error', title: 'Không thể thêm chủ đề', message: res?.error || 'Đã có lỗi xảy ra.' })
      }
    },
      ...
      try {
        ...
        if (res.success) {
          ...
          if (this.searchResults.length === 0) {
            this.searchMessage = 'Không tìm thấy địa điểm mới, có thể địa điểm đã có trong danh sách.'
          }
        } else {
