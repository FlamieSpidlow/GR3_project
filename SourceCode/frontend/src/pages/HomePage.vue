...
import { filterPlacesByCategoryOptions, getCategoryId, getCategoryLabel, getCategoryOptions, normalizeText } from '../utils/placeFilters'
...
export default {
  ...
  computed: {
    heroStats() {
      return [
        ...
        { value: '10+', label: 'loại địa điểm vui chơi' }
      ]
    },
    categoryCards() {
      return getCategoryOptions(this.allPlaces, this.categories)
        ...
        .map(category => {
          return {
            ...category,
            image: this.getCategoryImage(category.id, category.label)
          }
        })
    },
    filteredPlaces() {
      if (this.activeCategory === 'all') return this.allPlaces
      return filterPlacesByCategoryOptions(this.allPlaces, this.activeCategory, this.categoryCards)
    },
    featuredPlaces() {
  ...
  methods: {
    ...
    getPlaceImage(place) {
      ...
      return assetUrl((place.images && place.images[0]) || place.image || fallbackImage)
    },
