    ...
    <main class="site-main">
      ...
      <div v-else-if="place" class="place-container">
          ...
          <div class="info-section">
            ...
            <div class="quick-info">
              ...
              <div class="info-badge">
                <UserGroupIcon class="badge-icon" aria-hidden="true" />
                <span>{{ place.ageRange || '0-12' }} tuổi</span>
              </div>
              <div class="info-badge">
                <BanknotesIcon class="badge-icon" aria-hidden="true" />
            ...
            <div class="facilities-grid">
              <div class="facility-item" v-if="place.parking">
                <TruckIcon class="facility-icon" aria-hidden="true" />
              ...
              <div class="facility-item" v-if="place.food">
                <CakeIcon class="facility-icon" aria-hidden="true" />
                <div class="facility-info">
              ...
              <div class="facility-item" v-if="place.facilities">
                <BuildingOffice2Icon class="facility-icon" aria-hidden="true" />
                <div class="facility-info">
            ...
            <div class="details-list">
              <div class="detail-item">
                <MapPinIcon class="detail-icon" aria-hidden="true" />
                <span class="detail-text">{{ cleanAddress(place.address, place.name) || 'Địa chỉ không rõ' }}</span>
              </div>
              <div class="detail-item" v-if="place.phone">
                <PhoneIcon class="detail-icon" aria-hidden="true" />
                <a :href="telUrl(place.phone)" class="detail-link">{{ formatPhone(place.phone) }}</a>
              </div>
              <div class="detail-item" v-if="place.openingHours && place.openingHours.length">
                <ClockIcon class="detail-icon" aria-hidden="true" />
                <span class="detail-text">{{ formatOpeningHours(place.openingHours) }}</span>
              </div>
              <div class="detail-item" v-if="place.website">
                <GlobeAltIcon class="detail-icon" aria-hidden="true" />
                <a :href="normalizeWebsite(place.website)" target="_blank" rel="noopener" class="detail-link">{{ formatWebsite(place.website) }}</a>
              </div>
            </div>

            <div class="action-row">
              <div v-if="isFreePlace" class="free-entry-note">
                Miễn phí, không cần đặt vé trước
              </div>
        ...
        <div class="nearby-section" v-if="nearbyPlaces.length > 0">
          <h2>Địa điểm tương tự gần đây</h2>
            ...
            >
              ...
              <div class="nearby-info">
                ...
                <div class="nearby-meta">
                  <span v-if="nearby.rating" class="nearby-rating"><StarIcon class="nearby-icon" aria-hidden="true" /> {{ nearby.rating }}/5</span>
      ...
      <div class="ticket-modal">
        ...
        <form v-if="!createdTicketOrder" class="ticket-form" @submit.prevent="submitTicketOrder">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn-submit" :disabled="ticketSubmitting">
              {{ ticketSubmitting ? 'Đang tạo mã QR...' : 'Tiếp tục thanh toán' }}
            </button>
          </div>
        </form>
        <div v-else class="ticket-payment">
          ...
          <div class="payment-summary">
            <div>
              <span>Mã đơn</span>
              <strong>{{ bookingCode(createdTicketOrder) }}</strong>
            </div>
            <div>
              <span>Tổng tiền</span>
              <strong>{{ formatPrice(createdTicketOrder.totalAmount || createdTicketOrder.totalPrice) }}</strong>
            </div>
          </div>
          ...
          <div v-if="!ticketPaymentCompleted" class="payment-qr-wrap">
            ...
            <div v-else class="payment-qr-empty">
              Chưa tạo được mã QR thanh toán. Vui lòng kiểm tra cấu hình thanh toán.
            </div>
            <div v-if="createdTicketOrder.payment?.transferContent" class="transfer-content">
              Nội dung thanh toán: <strong>{{ createdTicketOrder.payment.transferContent }}</strong>
            </div>
            <div v-if="paymentQrImage" class="transfer-hint">
              Quét QR bằng ứng dụng ngân hàng. Sau khi thanh toán được xác nhận, vé điện tử sẽ xuất hiện trong mục Vé của tôi.
            </div>
          </div>
...
import { formatPhone, formatWebsite, normalizeWebsite, telUrl } from '../utils/infoFormatter'
import { bookingCode } from '../utils/displayLabels'
import {
  BanknotesIcon,
  BuildingOffice2Icon,
  CakeIcon,
...
export default {
  ...
  methods: {
      ...
      if (res.success) {
        ...
        if (payment.provider === 'payos') {
          ...
          if (!this.paymentQrImage) {
            this.ticketError = 'Chưa tạo được mã QR thanh toán. Vui lòng kiểm tra cấu hình thanh toán.'
            this.ticketSuccess = ''
            return
          }
          this.startPaymentStatusWatcher(res.data)
          this.ticketSuccess = 'Mã QR thanh toán đã được tạo. Vui lòng quét mã bằng ứng dụng ngân hàng để hoàn tất thanh toán.'
          return
        }
    ...
    async createPaymentQr(order) {
      ...
      if (payment.provider === 'payos') {
        const qrSource = payment.qrUrl || payment.payUrl || payment.transferContent || payment.orderRef || ''
        if (!qrSource) return
        if (/^data:image\//i.test(qrSource)) {
          this.paymentQrImage = qrSource
        } else {
          this.paymentQrImage = await QRCode.toDataURL(qrSource, {
            width: 240,
            margin: 2,
    ...
    startPaymentStatusWatcher(order) {
      ...
      this.paymentStatusTimer = window.setInterval(async () => {
        ...
        const isPaid = res.data.paymentStatus === 'paid' || res.data.status === 'paid'
        if (!isPaid) return
    ...
    async checkCurrentTicketPayment({ silentPending = false } = {}) {
      ...
      if (!isPaid) {
        if (!silentPending) {
          this.ticketError = ''
          this.ticketSuccess = 'Thanh toán chưa được xác minh. Vui lòng chờ hệ thống đối soát giao dịch.'
        }
        return false
      }
