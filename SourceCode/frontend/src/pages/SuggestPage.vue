      ...
      <div class="user-card tw-surface">
        ...
        <div class="user-info">
          <div class="user-heading">
            <h2>{{ user.name }}</h2>
            <a class="email" v-if="user.email" :href="mailtoUrl(user.email)">{{ formatEmail(user.email) }}</a>
          </div>
...
import { formatEmail, mailtoUrl } from '../utils/infoFormatter'
...
export default {
  ...
  computed: {
    ...
    filteredRecommendations() {
      return filterPlacesByCategoryOptions(this.recommendations, this.activeCategory, this.categoryOptions)
    },
    categoryOptions() {
...
.user-info .email { display:inline-block; margin:5px 0 0; color:#475569; font-size:1rem; line-height:1.35; text-decoration:none; }
.user-info .email:hover { color:#2563eb; text-decoration:underline; }
.profile-row {
  display: inline-flex;
  align-items: center;
  gap: 10px;
