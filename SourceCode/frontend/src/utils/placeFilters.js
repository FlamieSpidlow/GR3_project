...
export function getCategoryOptionLabel(categoryOptions, categoryId) {
  if (!categoryId || categoryId === 'all' || !Array.isArray(categoryOptions)) return ''
  const selected = categoryOptions.find(category => category.id === categoryId)
  return selected ? selected.label : ''
}

export function filterPlacesByCategoryOptions(places, categoryId, categoryOptions = []) {
  return filterPlacesByCategory(places, categoryId, getCategoryOptionLabel(categoryOptions, categoryId))
}
