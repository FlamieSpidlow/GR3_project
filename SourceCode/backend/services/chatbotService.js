...
const retrieveDocs = async ({ question, filterIds = [], userLocation = null, limit = DEFAULT_TOP_K }) => {
  const [places, reviews, tagFilters] = await Promise.all([
    Place.find({}).lean(),
    Review.find({}).populate('place', 'name lat lng').sort({ createdAt: -1 }).lean(),
...
const getGeminiApiKey = () => {
  ...
  if (!key) throw new Error('GEMINI_API_KEY is required in backend/.env')
  return key
}

const getGeminiModels = () => {
  const configured = String(process.env.GEMINI_MODEL || 'gemini-2.5-flash-lite')
    .split(',')
...
const callGemini = async ({ system, prompt, maxOutputTokens = 512, temperature = 0.2 }) => {
  ...
  for (const model of models) {
    ...
    if (res.ok) return extractGeminiText(data)
...
const generateFallbackAnswerFromDocs = (docs) => {
  const map = new Map()
  for (const doc of Array.isArray(docs) ? docs : []) {
    const meta = doc && doc.metadata ? doc.metadata : {}
    const id = meta.placeMongoId ? String(meta.placeMongoId) : ''
    const name = meta.placeName ? String(meta.placeName).trim() : ''
    if (id && name && !map.has(id)) map.set(id, name)
  }

  const names = Array.from(map.values()).slice(0, 3)
  if (names.length === 0) return NO_DATA_RESPONSE
  return `Bạn có thể tham khảo ${names.join(', ')}. Mình đang lấy dữ liệu từ danh sách địa điểm hiện có.`
}

const buildFollowUpSuggestions = ({ answer, places = [], docs = [] } = {}) => {
  if (answer === NO_DATA_RESPONSE) {
    return [
      'Gợi ý địa điểm có hoạt động ngoài trời',
      'Có địa điểm nào miễn phí không?',
      'Địa điểm nào phù hợp cho trẻ em?'
    ]
  }

  const firstPlace = Array.isArray(places) && places[0] && places[0].name ? places[0].name : ''
  if (firstPlace) {
    return [
      `Giá vé ${firstPlace} là bao nhiêu?`,
      `${firstPlace} mở cửa lúc mấy giờ?`,
      `Địa chỉ ${firstPlace} ở đâu?`
    ]
  }

  const firstDoc = Array.isArray(docs) ? docs.find(doc => doc && doc.metadata && doc.metadata.placeName) : null
  const placeName = firstDoc && firstDoc.metadata ? String(firstDoc.metadata.placeName || '').trim() : ''
  if (placeName) {
    return [
      `Giá vé ${placeName} là bao nhiêu?`,
      `${placeName} phù hợp độ tuổi nào?`,
      `Có tiện ích gì ở ${placeName}?`
    ]
  }

  return [
    'Gợi ý thêm vài địa điểm tương tự',
    'Có địa điểm nào gần tôi không?',
    'Địa điểm nào phù hợp đi cuối tuần?'
  ]
}
...
const answerQuestion = async (question, conversationId, userLocation) => {
  ...
  if (!sanitized) {
    return {
      ...
      conversationId: cid,
      places: [],
      suggestions: buildFollowUpSuggestions({ answer: NO_DATA_RESPONSE })
    }
  }
  ...
  if (smallTalkAnswer) {
    ...
    return {
      ...
      conversationId: cid,
      places: [],
      suggestions: buildFollowUpSuggestions({ answer: smallTalkAnswer })
    }
  }
  ...
  if (directAnswer) {
    ...
    const places = linkedPlace ? [linkedPlace] : []
    return {
      answer: directAnswer,
      conversationId: cid,
      places,
      suggestions: buildFollowUpSuggestions({ answer: directAnswer, places })
    }
  }
  ...
  if (directRecommendation) {
    ...
    return {
      answer: directRecommendation.answer,
      conversationId: cid,
      places,
      suggestions: buildFollowUpSuggestions({ answer: directRecommendation.answer, places })
    }
  }
  ...
  let finalAnswer = NO_DATA_RESPONSE
  try {
    ...
    finalAnswer = normalizeNoDataResponse(answer)
  } catch (err) {
    console.error('Chatbot Gemini fallback:', err && err.message ? err.message : err)
    finalAnswer = generateFallbackAnswerFromDocs(docs)
  }
  ...
  return {
    ...
    conversationId: cid,
    places: linkedPlaces,
    suggestions: buildFollowUpSuggestions({ answer: finalAnswer, places: linkedPlaces, docs })
  }
}
