...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user.password) {
      return res.status(400).json({ success: false, error: 'Tài khoản này đăng nhập bằng Google. Vui lòng chọn Đăng nhập với Google.' })
    }
...
router.post('/google', async (req, res) => {
  try {
    ...
    if (!credential) return res.status(400).json({ success: false, error: 'Thiếu thông tin đăng nhập Google' })
    ...
    res.status(err.status || 500).json({
      success: false,
      error: err.status === 503 ? 'Đăng nhập Google chưa được cấu hình' : 'Lỗi đăng nhập Google',
      details: err.message
    })
  }
})
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      return res.json({ success: true, message: 'Mã xác thực đã được tạo cho môi trường local', code })
    }
...
router.put('/profile', authenticate, async (req, res) => {
  try {
    ...
    if (!parentName && !address && !email && !avatar && phone === undefined) {
      return res.status(400).json({ success: false, error: 'Không có trường để cập nhật' })
    }

    const user = req.user
    if (email && email !== user.email) {
      const existing = await User.findOne({ email })
      if (existing) return res.status(400).json({ success: false, error: 'Email đã được sử dụng' })
      user.email = email
    }
    ...
    await user.save()
    res.json({ success: true, message: 'Cập nhật thành công', user: buildUserResponse(user) })
  } catch (err) {
...
router.put('/favorites/:placeId', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Lỗi khi cập nhật yêu thích', details: err.message })
  }
})
